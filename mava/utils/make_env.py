...
import jumanji
import matrax
from mava.coordsum import make as make_coordsum
...
_coordsum_registry = {"CoordSum": CoordSumWrapper}
...
def make_coordsum_env(config: DictConfig, add_global_state: bool = False) -> Tuple[MarlEnv, MarlEnv]:
    """
 

    """
    # Select the Matrax wrapper.
    wrapper = _coordsum_registry[config.env.scenario.name]

    # Create envs.
    task_name = config["env"]["scenario"]["task_name"]
    train_env = make_coordsum(task_name, **config.env.kwargs)
    eval_env = make_coordsum(task_name, **config.env.kwargs)
    train_env = wrapper(train_env, add_global_state)
    eval_env = wrapper(eval_env, add_global_state)
...
def make(config: DictConfig, add_global_state: bool = False) -> Tuple[MarlEnv, MarlEnv]:
    ...
    elif env_name in _coordsum_registry:
        return make_coordsum_env(config, add_global_state)
